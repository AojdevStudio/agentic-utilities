...
Default profile names used by the bundled CLI, all targeting a Bambu Lab P2S with a 0.4 mm nozzle. Override via `BAMBU_MACHINE_PROFILE` and the maps in `cli/profiles.ts` for other Bambu printers.
...
The maps live in `cli/profiles.ts` (`PROCESS_MAP`, `FILAMENT_MAP`). To support a different Bambu printer (X1C, P1S, A1, …):
...
The CLI doesn't yet expose `--infill` / `--walls` flags. To override infill density, wall count, or shell layers when remixing, write a tmp lean-process JSON that overrides the relevant keys (`sparse_infill_density`, `wall_loops`, `sparse_infill_pattern`, `top_shell_layers`, `bottom_shell_layers`) and pass it as the second `--load-settings` semicolon entry. The pattern matches `patchMachineProfile` in `cli/slicer.ts`.
