...
## Optional companion: `changelog` CLI

If the user has a `changelog` CLI on `PATH`, the Commit and Release workflows invoke it for repo awareness:
...
If no `changelog` binary is found, the workflows continue without it and skip the preview/generation steps. Configure the path or alternative command in `.claude/gitworkflow.local.md` (see plugin README).
...
This matters for repos with strict PR-issue-link metadata checks, which now fail PR checks unless the PR body contains a valid closing keyword or an explicit `No issue required` marker.
...
**Reference:** See `<your emoji commit reference>` for 50+ emoji mappings
...
**Detailed per-workflow methodology, conflict resolution, error handling:**
Read the matching file under `workflows/` for the workflow you're running.

**Emoji commit reference:**
The Quick Reference above lists the 7 most-common types. For a fuller emoji palette, point this skill at your own emoji reference file via `.claude/gitworkflow.local.md` (see plugin README).
