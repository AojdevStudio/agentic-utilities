...
You are auditing a `{{STACK}}` repo for "harness engineering" readiness — meaning: how well-equipped is this codebase for autonomous AI agents (Claude Code, Codex, Pi, or similar) to do end-to-end work without a human babysitting them?
...
- Any GitHub Actions that spawn AI agents on PRs?
...
- Agent harness metadata (`.claude/settings.json` hooks, `.mcp.json` MCP servers, `package.json#pi`, `.pi/skills/`)
