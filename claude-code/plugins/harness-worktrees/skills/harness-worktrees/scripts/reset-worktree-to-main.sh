...
LATEST_PR_LINE="(gh not installed)"
PR_IN_TARGET="n/a"
if command -v gh >/dev/null 2>&1; then
  if pr_row="$(gh pr list --state merged --limit 1 --json number,title,mergeCommit --jq '.[0] | "\(.number)\t\(.mergeCommit.oid)\t\(.title)"' 2>/dev/null)" && [[ -n "$pr_row" ]]; then
    IFS=$'\t' read -r PR_NUMBER PR_MERGE_SHA_FULL PR_TITLE <<<"$pr_row"
    if [[ -n "$PR_NUMBER" && -n "$PR_MERGE_SHA_FULL" ]]; then
      PR_MERGE_SHA="${PR_MERGE_SHA_FULL:0:7}"
      if git merge-base --is-ancestor "$PR_MERGE_SHA_FULL" "$TARGET_REF" 2>/dev/null; then
        PR_IN_TARGET="yes"
      else
        PR_IN_TARGET="no — not yet in $TARGET_REF"
      fi
      LATEST_PR_LINE="#$PR_NUMBER $PR_MERGE_SHA \"$PR_TITLE\""
    else
      LATEST_PR_LINE="(no merged PRs found)"
    fi
  else
    LATEST_PR_LINE="(no merged PRs or gh query failed)"
  fi
fi
...
WORKTREE RESET PLAN
  ...
  latest PR:      $LATEST_PR_LINE
  pr in target:   $PR_IN_TARGET
