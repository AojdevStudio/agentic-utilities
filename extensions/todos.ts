// @ts-nocheck
/**
 * Pi extension: file-backed todos (markdown + JSON front matter) with optional locks and a TUI.
 *
 * @remarks
 *
 * Storage defaults to `.pi/todos` or the directory in `PI_TODO_PATH`. Each item is `<id>.md`; an
 * optional `<id>.lock` marks an active edit session.
 *
 * **File shape** — Leading JSON (not YAML) holds
 * `{ id, title, tags, status, created_at, assigned_to_session }`, then a blank line, then markdown body.
 *
 * **Settings** — `<todo-dir>/settings.json` may set `gc` and `gcDays` to delete old closed todos on startup.
 *
 * **UX** — `/todos` opens the visual manager; the agent can also use the `todo` tool.
 *
 * **Source layout** (top to bottom): id/search helpers → TUI components → disk, locks, GC →
 * formatting → mutations → `registerTool("todo")` and `registerCommand("todos")`.
 */
...
/** Stale lock threshold: locks older than this may be removed (interactive steal flow). */
const LOCK_TTL_MS = 30 * 60 * 1000;

/**
 * JSON front matter for a todo file (serialized before the markdown body).
 *
 * @remarks
 * Persisted as the opening `{ ... }` block in `<id>.md`.
 */
...
/**
 * Complete todo record including long-form markdown body.
 *
 * @remarks
 * Used by tool `get`/`update`/`append`, filesystem I/O, and the read-only detail overlay.
 */
...
/**
 * Serialized contents of `<id>.lock` while a todo file is exclusively locked.
 */
interface LockInfo {
  id: string;
  pid: number;
...
/**
 * Optional `<todo-dir>/settings.json` payload.
 *
 * @remarks
 * Controls garbage collection of closed todos by age (`gc`, `gcDays`).
 */
...
/**
 * Abstraction over Pi keybindings so TUI code stays independent of concrete key chords.
 */
...
/**
 * TypeBox schema for the `todo` tool parameters (validated at invoke time).
 */
...
/**
 * Todo id and list utilities: formatting, validation, sort order, and fuzzy filtering.
 *
 * @remarks
 * Display ids use the `TODO-` prefix; on disk filenames are raw hex. Closed todos sort after open;
 * assigned open items sort before unassigned.
 */
...
/**
 * Interactive todo list for the `/todos` command.
 *
 * @remarks
 * Search input, fuzzy-filtered list, Enter to open actions; `Ctrl+Shift+W` / `Ctrl+Shift+R` for quick work/refine.
 */
...
/**
 * Action menu for a selected todo (view, work, refine, close/reopen, release, copy, delete).
 */
...
/**
 * Confirmation dialog before deleting a todo file.
 */
...
/**
 * Centered overlay showing the todo body as rendered markdown.
 *
 * @remarks
 * Esc returns; Enter triggers the "work on this todo" flow.
 */
...
/**
 * Absolute path to the todo directory for a session/workspace.
 *
 * @remarks
 * Uses `PI_TODO_PATH` (relative to `cwd`) when set; otherwise `.pi/todos` under `cwd`.
 * Parsing, locks, and GC live in sibling helpers below.
 *
 * @param cwd - Directory used to resolve relative todo paths.
 */
...
/**
 * Try to create an exclusive lock file for a todo before mutating it.
 *
 * @remarks
 * If the lock exists and is newer than {@link LOCK_TTL_MS}, returns an error; in UI mode the user may
 * confirm stealing a stale lock.
 *
 * @returns A function that removes the lock, or an `{ error: string }` if locking failed.
 */
...
/**
 * Executes `fn` while holding the todo lock, releasing it in a `finally` block.
 *
 * @typeParam T - Successful return type of `fn`.
 *
 * @returns The result of `fn`, or `{ error: string }` if the lock could not be acquired.
 */
...
/**
 * Registers the todos extension with the Pi host.
 *
 * @remarks
 * On `session_start`, ensures the todo directory exists and runs GC when enabled. Registers the `todo`
 * tool and the `/todos` slash command.
 *
 * @param pi - Extension host API (tools, commands, lifecycle).
 */
