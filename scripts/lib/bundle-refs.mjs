 ...
 * at one, so a coincidental prefix of a longer path (e.g.
 * `assets/logo.png.bak` or `assets/logo.png/child.bin`) can't be truncated
 * into a match for a real asset (`assets/logo.png`) that happens to exist.
 * The terminal boundary must reject `/` as well as word/dot/hyphen
 * characters — `/` is itself part of the accepted path alphabet, so a
 * shorter match immediately followed by more path is still a truncation,
 * not a genuine end of token. Leading `./` and `../` segments are captured
 * as part of the reference itself, not silently dropped or left unmatched,
 * so both a safe same-directory prefix and real traversal are visible
